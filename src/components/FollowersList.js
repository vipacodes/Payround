...
export default function FollowersList({ userId, userEmail, userName, onClose, highlight }) {
  ...
  // 🎯 Deep-linked from a follower notification? Scroll that person into view & let them glow
  useEffect(() => {
    if (!people || !hlLower) return;
    const el = document.getElementById(`follower-${hlLower}`);
      ...
      <div className="bg-white w-full sm:max-w-sm sm:mx-4 rounded-t-2xl sm:rounded-2xl max-h-[80vh] flex flex-col overflow-hidden" onClick={e => e.stopPropagation()}>
        ...
        <div className="overflow-y-auto">
          {people === null ? (
            ...
            people.map(({ email, u }) => {
              const isHl = !!hlLower && email === hlLower;
              return (
              <button
                id={`follower-${email}`}
                key={email}
                onClick={() => { if (u?.id) { onClose(); router.push(`/users/${u.id}`); } }}
                className={`w-full flex items-center gap-3 px-4 py-3 transition-colors text-left ${isHl ? 'bg-primary-50 ring-2 ring-primary-400 rounded-xl my-0.5 hover:bg-primary-100' : 'hover:bg-gray-50'}`}
              >
                {u?.profile_pic ? (
                ...
                <span className="flex-1 min-w-0">
                  ...
                  {isHl && <span className="inline-flex items-center gap-1 mt-0.5 text-[10px] font-bold bg-primary-600 text-white px-2 py-0.5 rounded-full">🎉 just followed you</span>}
                </span>
