          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {groups.length > 0 ? groups.slice(0, 3).map(group => (
              ...
              <div className="col-span-3 bg-white rounded-2xl border border-dashed p-12 text-center">
                ...
                <p className="text-sm text-gray-500 mt-1">Be the first to create a group and start saving together.</p>
                <button onClick={() => router.push('/groups/create')} className="mt-4 bg-primary-600 text-white px-6 py-2 rounded-xl text-sm">Create Group</button>
              </div>
            )}
          </div>
